...
class MamentorCrawler(BaseCrawler):
    ...
    def _list_url(self, bo: str, page: int, keyword: str = "", search_field: str = "") -> str:
        params = {"bo_table": bo, "page": page}
        if keyword:
            params.update({
                "sfl": search_field or "wr_subject||wr_content",
                "stx": keyword,
                "sop": "and",
            })
        return f"{MAMENTOR_BASE}/bbs/board.php?{urlencode(params, safe='|')}"
        ...
        for bo in boards:
            ...
            for keyword in search_terms:
                for search_field in search_fields:
                    ...
                    on_log(f"[마멘토] 게시판 '{bo}' 수집 시작{suffix} ({cfg.pages_per_board}페이지)")
                    for page in range(1, cfg.pages_per_board + 1):
                        if should_stop(cfg):
                            on_log("[마멘토] 중지 요청 감지")
                            break
                        url = self._list_url(bo, page, keyword, search_field)
                        try:
                            html = self._get(url)
